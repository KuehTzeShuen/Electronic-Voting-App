...
export default function OngoingPollsPage() {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative z-10 w-full flex flex-row items-center justify-center px-8 pt-8 transition-opacity duration-500"
      >
        <input
          type="text"
          placeholder="Search"
          className="rounded-full px-4 py-2 bg-[#1a1a1c] text-white text-sm font-semibold outline-none"
          style={{ minWidth: 200 }}
          disabled
        />
      </div>
      {/* Poll cards */}
      <div className="relative z-10 w-full flex flex-col gap-6 px-4 pt-8 items-center">
        {polls.map((poll, idx) => {
          ...
          return (
            <div
              ...
              className={`
          rounded-3xl ${poll.color} p-5 flex flex-col gap-3 relative
          transition-all duration-700
